...
def clean_data(file_path):
    ...
    print(f"Initial dataset shape: {cleaned_data.shape}")
    ...
    for column in cleaned_data.columns:
        ...
        if cleaned_data[column].dtype in [np.int64, np.float64]:
            value_counts = cleaned_data[column].value_counts(normalize=True)
    ...
    cleaned_data[columns_to_modify] = 0

    # Move modified columns to the end of the DataFrame
    print("Rearranging columns to move modified columns to the end...")
    columns_order = [col for col in cleaned_data.columns if col not in columns_to_modify] + columns_to_modify
    ...
    cleaned_data['timestamp'] = pd.to_datetime(cleaned_data['message_timestamp'], errors='coerce')

    # Convert timestamp to ordinal value (milliseconds since Unix epoch)
    cleaned_data['ordinal_timestamp'] = cleaned_data['timestamp'].apply(lambda x: x.value if pd.notna(x) else np.nan)
    ...
    day_columns = pd.get_dummies(cleaned_data['day'], prefix='day')
    cleaned_data = pd.concat([cleaned_data, day_columns], axis=1)

    # Drop the original timestamp-related columns
    cleaned_data = cleaned_data.drop(['message_timestamp', 'timestamp', 'day'], axis=1)
    ...
    for col in cleaned_data.columns:
        if col not in numeric_columns and cleaned_data[col].dtype == bool:
